...
returned in the `DataAddress`. The `accessToken` MAY conform to the [Access Token Claims](#access-token-claims)
profile, in which case the `expiresIn` property MUST be consistent with the token's `exp` claim.
...
If the original access token conforms to the [Access Token Claims](#access-token-claims) profile, the newly issued
access token MUST carry the same claim set as described in
